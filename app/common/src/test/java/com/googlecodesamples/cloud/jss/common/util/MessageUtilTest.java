...
public class MessageUtilTest {
  ...
  private static final String WARN_MSG_GCP_LOCATION = "environment variable 'GOOGLE_CLOUD_LOCATION' has not been set, test skipped.";
  ...
  public void testConvertToPubSubMessage() throws IOException {
    ...
    for (int i = 0; i < LOOP_COUNT; i++) {
      ...
      assertThat(messageJsonObj.get("battery_capacity_kwh").getAsFloat()).isIn(EXPECTED_BATTERY_CAPACITY);
      assertThat(messageJsonObj.get("battery_level_start").getAsFloat()).isAtLeast(EXPECTED_MIN_BATTERY_PERCENTAGE);
      assertThat(messageJsonObj.get("battery_level_start").getAsFloat()).isAtMost(EXPECTED_MAX_BATTERY_PERCENTAGE);
    }
  }

  @Test
  public void testConvertToAvroEvent() throws IOException {
    String content = "{\"session_id\":\"00c180a3-afbb-4766-a6ed-086a983e353d\"," +
